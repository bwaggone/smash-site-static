...
def write_upset(p1, p2, upsets_file, t_slug, t_name):
    upsets_file.write(t_name + ',' + t_slug + ',' + p1 + ',' + p2 + '\n')
    return

#Get Players on the Official Ranking List
ranks = defaultdict(int)
first = 1
with open("../csv/Melee-rankings.csv") as stream:
...
upsets_file = "upsets.csv"
up_file = open(upsets_file,"w")
        ...
        for set_data in incsv:
            ...
            if((p1rank == 0 or p2rank == 0) and not(p1rank == p2rank)):
                ...
                if((p1rank == 0 and (int(set_data[2]) == 0)) or (p2rank == 0 and (int(set_data[2]) == 1))):
                    write_upset(set_data[0], set_data[1], up_file, tourney[0], tourney[3])
